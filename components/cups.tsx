...
export default function Cups({ count, initialCups, session, isLiked = false, type = 'all', search }: Props) {
  ...
  const cupsWithAD: (CupWithUser | 'adsense')[] = []

  cups.forEach((cup, index) => {
    if (index % 6 === 0) {
      cupsWithAD.push('adsense')
    }
    ...
    <section className='sm:px-2 pb-20'>
      <ResponsiveMasonry className='px-1 w-full' columnsCountBreakPoints={{ 0: 2, 760: 3, 1100: 4, 1400: 5, 1700: 6 }}>
        <Masonry gutter='2px' className='pb-20 w-full'>
          {cupsWithAD.map((cup, index) => {
            if (cup === 'adsense') {
              return <GoogleAdsense key={index} />
            }
                ...
                <div className='h-full flex flex-col px-1 pt-2 pb-1.5 break-all gap-1'>
                  ...
                  <div className='flex items-center gap-1'>
                    <Link
                      ...
                      className={cn(
                        buttonVariants({ className: 'text-xs text-[11px] px-2 flex-1', size: 'sm' }),
                        'h-8'
                      )}
                    >
